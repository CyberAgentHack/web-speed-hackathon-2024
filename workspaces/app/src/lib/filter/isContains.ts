...
export function isContains({ query, target }: Params): boolean {
  ...
  TARGET_LOOP: for (let offset = 0; offset <= target.length - query.length; offset++) {
    for (let idx = 0; idx < query.length; idx++) {
      // 1文字ずつ localeCompare で比較する
      const targetChar = target[offset + idx]!;
      const queryChar = query[idx]!;
      // sensitivity: 'accent' を指定し、濁点・半濁点・アクセントを無視した比較を行う
      if (targetChar.localeCompare(queryChar, 'ja', { sensitivity: 'accent' }) !== 0) {
        continue TARGET_LOOP;
      }
    }
    // query のすべての文字が含まれていたら true を返す
